...
def train(cfg):
    ...
    dataloader = torch.utils.data.DataLoader(
        train_set, batch_size=hps.batch_size, shuffle=True
    )
    testloader = torch.utils.data.DataLoader(
        test_set, batch_size=hps.batch_size, shuffle=False
    )
        ...
        for i, (data, target) in enumerate(dataloader):
            ...
            if i % 10 == 0:
                print(
                    ...
                    + "{:5}".format(i * len(data))
                    + "/"
                    + "{:5}".format(len(train_set))
                    + " ("
                    + "{:3.0f}".format(100 * i / len(dataloader))
                    + "%)]  Loss: "
    ...
    print("Saved model files in " + "'gs://model_checkpoints_group24/trained_model.pt")
