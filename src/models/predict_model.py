...
def predict(img=None, model_state=None):
    ...
    if img is None:
        ...
        input = torchvision.io.read_image(
            args.load_input_from,
            torchvision.io.ImageReadMode.GRAY).float()
    else:
        input = torchvision.io.read_image(
            img, torchvision.io.ImageReadMode.GRAY).float()
    ...
    if img is None:
        print(
            f"The predicted label is: {pred.item()}" +
            "with probability {preds.max().item():.2f}"
        )
    else:
        return (f"The predicted label is: {pred.item()}" +
                f"with probability {preds.max().item():.2f}"
                )
